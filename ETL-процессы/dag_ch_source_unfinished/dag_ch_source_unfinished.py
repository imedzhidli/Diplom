...
scr0 = join(sql_path, 'truncate_ch_source_unfinished.sql')
scr1 = join(sql_path, 'insert_ch_source_unfinished.sql')
...
with DAG(
    ...
    task_insert_ch_source_unfinished = PythonOperator(
        task_id='insert_ch_source_unfinished',
        python_callable=transfer_postgresql_to_clickhouse,
        op_kwargs={'sql_clean_script_path': scr0,
                   'sql_script_path': scr1,
                   'clickhouse_table_name': 'source_unfinished',
                   }
    )
...
start >> \
    task_insert_ch_source_unfinished >> \
    end
