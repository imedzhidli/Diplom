...
scr0 = join(sql_path, 'truncate_ch_object_addresses.sql')
scr1 = join(sql_path, 'insert_ch_object_addresses.sql')
...
with DAG(
    ...
    task_insert_ch_object_addresses = PythonOperator(
        task_id='insert_ch_object_addresses',
        python_callable=transfer_postgresql_to_clickhouse,
        op_kwargs={'sql_clean_script_path': scr0,
                   'sql_script_path': scr1,
                   'clickhouse_table_name': 'object_addresses',
                   }
    )
...
start >> \
    task_insert_ch_object_addresses >> \
    end
